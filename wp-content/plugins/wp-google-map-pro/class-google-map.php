...
<div id='.$this->divID.' style="width:'.$width.'; height:'.$height.';"></div>';
	...
	{
		$main_listing_div = '<div class="wpgmp_listing_container" rel="'.$this->divID.'">'.do_shortcode("[put_wpgm_locations id=".$this->divID."_".$this->map_id."  filters='".$this->filters."' ]").'</div>';
    }
}
...
{	
				    ...
				    if($this->map_controls['wpgmp_direction_tab']==true)
		{
				    ...
				    $this->code.='<div id="wpgmp_dir_tab'.$this->map_id.'" style="'.$default_styles['hide_direction_tab'].'">
							<div class="wpgmp_direction_container">
								...
								<input type="submit" value="'.__("Find Direction","wpgmp_google_map").'" class="wpgmp_find_dir_button" onclick="wpgmp_calculate_route('.$this->map_id.');">
							</div>
		...
		$default_styles['hide_route_tab']="display:none";
		if($this->map_controls['wpgmp_unit_selected']=='km')
			$nearby_unit = __('KM',"wpgmp_google_map");
		else
			$nearby_unit = __('Miles',"wpgmp_google_map");
		...
		<div id="wpgmp_nearby_tab'.$this->map_id.'" style="'.$default_styles['hide_nearby_tab'].'">
							<div class="wpgmp_direction_container">
								...
								<input type="hidden" id="nearbyunitsystem'.$this->map_id.'" value="'.$this->map_controls['wpgmp_unit_selected'].'" />
								 <div><span>'.__("Current Location",'wpgmp_google_map').'</span> :<span class="wpgmp_mcurrent_loction" title="'.__("Take Current Location","wpgmp_google_map").'" id="nearby_current_location" style="width:150px; margin:0px;" </span></div>
								 <div><span class="wpgmp_set_current_location"></span></div>
								<input id="wpgmp_radius'.$this->map_id.'" placeholder="Radius (miles)" type="text" class="input" value="25" size=8> '.$nearby_unit.' 
								</form>
...
if( $this->marker[$i]['info']!='' )
{
	...
	$infos_mess = apply_filters("wgmp_infowindow_content",$infos_mess,$this->marker[$i]['location_id'],$this->map_id);
	     ...
	     if(isset($this->map_controls['infowindow_openoption']) and $this->map_controls['infowindow_openoption'] == "mousehover")
	     {
