...
add_action( 'wpmu_new_blog', 'wpgmp_on_create_blog', 10, 6 );
...
function wpgmp_on_create_blog( $blog_id, $user_id, $domain, $path, $site_id, $meta ) {
	if ( is_plugin_active_for_network( 'wp-google-map-pro/wp-google-map-pro.php' ) ) {
...
function wpgmp_delete_blog( $tables ) {
	global $wpdb;
	$tables[] = $wpdb->prefix . 'map_locations';
	$tables[] = $wpdb->prefix . 'create_map';
	$tables[] = $wpdb->prefix . 'group_map';
	$tables[] = $wpdb->prefix . 'map_routes';
	return $tables;
}
add_filter( 'wpmu_drop_tables', 'wpgmp_delete_blog' );
/**
		...
		if ( is_array( $all_custom_markers ) ) {
			...
			foreach ( $all_custom_markers as $marker ) {
				...
				if ( is_array( $category_data ) ) {
					...
					if ( isset( $marker['id'] ) and $marker['id'] != '' ) {
