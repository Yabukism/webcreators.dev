...
function wpgmp_display_map_location($atts, $content=null){
 ...
 //split map id and container id here
 $container_info = explode('_',$atts["id"]);

 if($container_info[0])
 	$container_id = $container_info[0];
 
 if($container_info[1])
 	$map_id = $container_info[1];
...
function wpgmp_get_map_location_ajax() {
	...
	{
		$options['container_id']=$_POST['container_id'];
	}
	else
	$options['container_id']='map2';
	...
	if(@$_POST['do_export']=='true')
	{
		...
		wpgmp_get_map_location($map_id,$options,$options['container_id']);
		die();
	}
    else
    echo wpgmp_get_map_location($map_id,$options,$options['container_id']);

	die(); // this is required to return a proper result
}

function wpgmp_get_map_location($map_id, $options='',$container="map2"){
   ...
   $map_locations = new Wpgmp_Pagination($query,$locations_per_page,$options['page'],'"'.$container."_".trim($map_id).'"');
...
if(is_array($all_custom_markers ) and !empty($all_custom_markers ))
{
  ...
  $map_locations_records = array_slice($map_locations_records,$staringlocation,$locations_per_page);
   $content ="";
	...
	$variables = array(
		"marker_link" => 'javascript:open_current_location(marker'.$map_locations_record->location_id.$options['container_id'].')',
