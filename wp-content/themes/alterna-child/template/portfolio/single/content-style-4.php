...
?>
<article id="post-<?php the_ID(); ?>" <?php post_class('post-entry single-portfolio-style-4'); ?> itemscope itemtype="http://schema.org/CreativeWork">
  <div class="row">
   <div class="single-portfolio-left-content col-lg-8 col-md-6 col-sm-6" >
    ...
    <div class="portfolio-preview-link">
      <a href="<?php echo esc_url(penguin_get_post_meta_key('portfolio-link')); ?>" class="btn btn-block btn-theme no-margin"><?php

        if(penguin_get_post_meta_key('portfolio-link-name') != ""){
         echo penguin_get_post_meta_key('portfolio-link-name');
       }else{
         _e('PREVIEW NOW','alterna');
       }
       ?></a>
     </div>
     <?php } ?>
